...
export default function SubscribePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase.from("talents").select("*").eq("user_id", auth.user.id).single();
  ...
  async function handleSubscribe(planType: "monthly" | "annual") {
    ...
    try {
      ...
      const razorpay = new window.Razorpay({
        ...
        theme: { color: "#5B21B6" },
        handler: () => {
          // Final confirmation arrives via the Razorpay webhook, which flips
          // talents.subscription_status to 'active'. Redirect back for now.
